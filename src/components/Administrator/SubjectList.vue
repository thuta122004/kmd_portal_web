      ...
      <table class="w-full text-left text-sm table-fixed">
        <thead class="bg-white/5 text-slate-400 uppercase text-[10px] tracking-wider">
          <tr>
            ...
            <th class="p-4 font-medium w-1/3">Code</th>
        ...
        <tbody class="divide-y divide-white/5">
          <tr v-if="isLoading">
            <td colspan="4" class="p-10 text-center text-slate-500">Loading subjects...</td>
          </tr>
          <template v-else-if="paginatedSubjects.length > 0">
            <tr v-for="sub in paginatedSubjects" :key="sub.id" class="text-slate-300">
              <td class="p-4 truncate cursor-pointer hover:text-blue-400" @click="openModal(sub)">
                {{ sub.name }}
              </td>
              <td class="p-4 text-slate-500 uppercase">{{ sub.code }}</td>
              <td class="p-4">
                <button
                  @click="handleToggleStatus(sub)"
                  :class="[
                    'w-8 h-4 rounded-full transition-colors relative',
                    sub.status === 'active' ? 'bg-blue-500' : 'bg-slate-700',
                  ]"
                >
                  <span
                    :class="[
                      'absolute top-1 w-2 h-2 rounded-full bg-white transition-all',
                      sub.status === 'active' ? 'left-5' : 'left-1',
                    ]"
                  ></span>
                </button>
              </td>
              <td class="p-4 text-right">
                <button @click="openModal(sub)" class="text-slate-500 hover:text-white transition">
                  Edit
                </button>
              </td>
            </tr>
          ...
          <tr v-else>
            <td colspan="4" class="p-10 text-center text-slate-500 italic">No records found.</td>
          </tr>
        </tbody>
      </table>
    </div>
...
const lastPage = computed(() => Math.ceil(filteredSubjects.value.length / itemsPerPage) || 1)
const paginatedSubjects = computed(() =>
  filteredSubjects.value.slice(
...
const handleToggleStatus = async (s) => {
  await api.patch(`/subjects/${s.id}/toggle`)
